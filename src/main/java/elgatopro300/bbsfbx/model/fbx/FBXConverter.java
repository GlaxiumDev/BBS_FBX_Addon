...
public class FBXConverter
{
    ...
    public static BOBJData convert(AIScene scene)
    {
        ...
        Map<Integer, Matrix4f> meshTransforms = collectMeshTransforms(rootNode, meshNodeNames, nodeParents);
        ...
        else
        {
            ...
            for (int i = 0; i < numMeshes; i++)
            {
                ...
                globalArmature.addBone(new BOBJBone(globalArmature.bones.size(), objectName, parentName, boneRest));
            }
        }
        ...
        if (scene.mNumAnimations() > 0 && !skinnedBones.isEmpty())
        {
            ...
            /* Bind-pose local transforms from each bone's inverse-bind (offset)
             * matrix - the SAME source the renderer uses to build relBoneMat.
             * Diffing against these (instead of the raw node transform, which
             * Blender may bake to frame 0) keeps a held pose consistent with the
             * bind pose, so constant "folded" poses render instead of snapping. */
            Map<String, Matrix4f> bindLocals = computeBindLocals(skinnedBones, globalArmature);

            processAnimations(scene, actions, globalArmature, nodeLocals, bindLocals, globalScale[0]);
        }
    ...
    private static void processMesh(AIScene scene, AIMesh aiMesh, int meshIndex, List<Vertex> vertices, List<Vector2d> textures, List<Vector3f> normals, List<BOBJMesh> meshes, BOBJArmature armature, float scaleFactor, Matrix4f rootCorrection, float offsetX, float offsetY, float offsetZ, Map<Integer, Matrix4f> meshTransforms, String objectBoneName)
    {
        ...
        if (aiNormals != null)
        {
            while (aiNormals.remaining() > 0)
            {
                ...
                {
                    // Skinned meshes: vertices were pre-rotated by meshRotationOnly,
                    // so normals MUST get the same rotation or lighting desyncs from
                    // the geometry (dark on -Y, bright on +Y).
    ...
    /**
     * Computes each skinned bone's bind-pose LOCAL transform (parent-relative)
     * from its inverse-bind (offset) matrix, mirroring how BOBJArmature builds
     * relBoneMat. A bone that merely holds its bind pose then yields an identity
     * delta and renders exactly where the skin expects it.
     */
    private static Map<String, Matrix4f> computeBindLocals(Map<String, AIBone> skinnedBones, BOBJArmature armature)
    {
        Map<String, Matrix4f> worldBind = new HashMap<>();
        for (Map.Entry<String, AIBone> entry : skinnedBones.entrySet())
        {
            Matrix4f offset = toMatrix4f(entry.getValue().mOffsetMatrix());
     ...
     */
    private static void processAnimations(AIScene scene, Map<String, BOBJAction> actions, BOBJArmature armature, Map<String, Matrix4f> nodeLocals, Map<String, Matrix4f> bindLocals, float globalScale)
    {
    ...
    private static Map<Integer, Matrix4f> collectMeshTransforms(AINode rootNode, Map<Integer, String> meshNodeNames, Map<String, String> nodeParents)
    {
